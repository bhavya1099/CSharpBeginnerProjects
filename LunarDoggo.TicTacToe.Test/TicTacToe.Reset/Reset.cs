...
Scenario 4: Resetting the game board after a player has won
  Details:
    TestName: TestResetAfterPlayerWin
    Description: Verify that calling the Reset method resets the game board after a player has won.
  Execution:
    Arrange: Create a new instance of the GameBoard class. Set some tiles to non-zero values to simulate a player win.
    Act: Call the Reset method.
    Assert: Check that all tiles in the tiles array are set to 0.
  Validation:
    The assertion aims to verify that the Reset method can reset the game board to a clean state after a player has won. This is important to ensure that the game can be restarted after a win without any lingering occupied tiles.

Scenario 5: Resetting the game board when all tiles are already occupied
  Details:
    TestName: TestResetOnFullBoard
    Description: Verify that calling the Reset method resets the game board when all tiles are already occupied.
  Execution:
    Arrange: Create a new instance of the GameBoard class. Set all tiles to non-zero values to simulate a full board.
    Act: Call the Reset method.
    Assert: Check that all tiles in the tiles array are set to 0.
  Validation:
    The assertion aims to verify that the Reset method can reset the game board to a clean state when all tiles are already occupied. This is important to ensure that the game can be restarted after a draw without any lingering occupied tiles.
...
namespace TicTacToe.Test
{
    ...
    public class ResetTest
    {
        ...
        [Test]
        [Category("valid")]
        public void TestResetAfterPlayerWin()
        {
            var gameBoard = new GameBoard();
            gameBoard.tiles[0, 0] = 1;
            gameBoard.tiles[0, 1] = 1;
            gameBoard.tiles[0, 2] = 1;
            gameBoard.Reset();
            for (int x = 0; x < gameBoard.tiles.GetLength(0); x++)
            {
                for (int y = 0; y < gameBoard.tiles.GetLength(1); y++)
        ...
        [Test]
        [Category("valid")]
        public void TestResetOnFullBoard()
        {
            var gameBoard = new GameBoard();
            gameBoard.tiles[0, 0] = 1;
            gameBoard.tiles[0, 1] = 2;
            gameBoard.tiles[0, 2] = 1;
            gameBoard.tiles[1, 0] = 2;
            gameBoard.tiles[1, 1] = 1;
            gameBoard.tiles[1, 2] = 2;
            gameBoard.tiles[2, 0] = 1;
            gameBoard.tiles[2, 1] = 2;
